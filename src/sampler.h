...
namespace Sampler {
  ...
  // uniform sample Mult(1/K), result [K1, K2]
  inline int uni_sample(int K1, int K2) {
	int k = rand() % (K2 - K1);
	return k + K1;
  }
  ...
  // p is p(1)
  inline bool Bern_sample(double p) {
	double u = double(rand()) / RAND_MAX;
	return (u < p);
  }

  // return counts sampling from multi-normial distribution p
  // the counts sum to N
